...
// MTD (Month-To-Date) sales and performance metrics with KPI cards matching dashboard style
...
class MonthlySummaryScreen {
    ...
    async load() {
        ...
        if (window.loadingOverlay) {
            window.loadingOverlay.show('Loading monthly summary data...');
        }
    ...
    async loadData(pharmacyId, date) {
        ...
        // Calculate the "through" date for previous year (same day of month)
        const mtdDay = currentDate.getDate();
        ...
        // Load MTD data and product lists in parallel
        const [mtdData, prevYearMtdData, bestSellersData, worstGpData] = await Promise.all([
            window.api.getMTD(pharmacyId, month, date),
            window.api.getMTD(pharmacyId, prevYearMonth, prevYearThroughDate).catch(e => { 
                console.warn('Previous year MTD error:', e); 
                return null; 
            }),
    ...
    render() {
        ...
        if (!this.data || !this.data.mtdData) {
            this.showEmptyState('No MTD data available for selected date');
            return;
        }

        const { mtdData, prevYearMtdData } = this.data;

        // Extract values from MTD data
        const turnover = Number(mtdData.turnover || 0);
        const prevYearTurnover = prevYearMtdData ? Number(prevYearMtdData.turnover || 0) : 0;
        const prevYearYear = prevYearMtdData ? new Date(prevYearMtdData.period_end || this.data.date + 'T00:00:00').getFullYear() : null;
        ...
        // Get transaction count from MTD data
        const transactionCount = Number(
            mtdData.transaction_count ||
            mtdData.transactions ||
            mtdData.txn_count ||
            0
        );
        ...
        const basketCard = this.formatBasketCard(transactionCount, turnover);

        // Get dispensary and frontshop turnover from MTD data
        const dispensaryTurnover = Number(mtdData.dispensary_turnover || 0);
        const frontshopTurnover = Number(mtdData.frontshop_turnover || 0);
        ...
        // Get scripts data from MTD data
        const scriptsQty = Number(mtdData.scripts_qty || 0);
        const avgScriptValue = scriptsQty > 0 ? dispensaryTurnover / scriptsQty : 0;

        // Get purchases and cost of sales from MTD data
        const purchases = Number(mtdData.purchases || 0);
        const costOfSales = Number(mtdData.cost_of_sales || 0) || (turnover > 0 && gpValue > 0 ? turnover - gpValue : 0);
        ...
        mainContent.innerHTML = `
            <div class="dashboard-container">
                <div class="dashboard-top-cards">
                    ...
                    ${this.renderPurchasesCard(purchases, costOfSales)}
                    ${this.renderTopCard('GP%', gpPercent, 'percentage', gpCard)}
                    ${this.renderTopCard('Basket', this.calculateBasketSize(turnover, transactionCount), 'currency', basketCard)}
                </div>
    ...
    formatBasketCard(transactionCount, turnover) {
        let secondary = { text: '—', className: '' };
    ...
    renderPurchasesCard(purchases, costOfSales) {
        ...
        const costOfSalesFormatted = costOfSales === 0 ? '—' : 
            new Intl.NumberFormat('en-ZA', {
                style: 'currency',
                currency: 'ZAR',
                minimumFractionDigits: 0,
    ...
    calculateBasketSize(turnover, transactions) {
    ...
    showEmptyState(message = 'Select a date to view monthly summary') {
        const mainContent = document.querySelector('.content-area');
        if (mainContent) {
            mainContent.innerHTML = `
                <div class="dashboard-container">
        ...
        if (mainContent) {
            mainContent.innerHTML = `
                ...
                <div class="error-state">
                    <h2>Error Loading Monthly Summary</h2>
    ...
    downloadTopSellersPdf() {
        ...
        // Save the PDF
        doc.save(`top-sellers-${fromDate}-to-${toDate}.pdf`);
    }
    ...
    downloadLowGpPdf() {
        ...
        // Save the PDF
        const filename = `low-gp-products-${fromDate}-to-${toDate}-below-${this.lowGpThreshold}percent.pdf`;
        doc.save(filename);
    }
...
// Export class
window.MonthlySummaryScreen = MonthlySummaryScreen;
